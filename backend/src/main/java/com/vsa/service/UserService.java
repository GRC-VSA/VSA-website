 ...
 *
 * <p>
 * Handles user registration, email verification, login, and password
 * management. Includes integration with email service for sending verification
 * and reset emails.
 *
 * <p>
 * Email verification is code-based rather than link-based: the user is emailed
 * a short code and types it back into the page they started on, so opening the
 * email on a phone doesn't strand the signup on a different device. The
 * mechanics mirror {@code RegistrationService}'s event-signup verification —
 * hashed single-use code, expiry, resend cooldown, and a cap on wrong guesses.
 *
...
public class UserService {
     ...
     * @param emailOutboxService Service for queuing emails for asynchronous
     * delivery
     ...
     *
     * <p>
     * If the email already belongs to an account that was created but never
     * verified, that pending row is reused and reissued a fresh code rather
     * than rejected. Without this, a user who closed the tab before verifying
     * could never sign up with that address again: registration would say the
     * email exists, and login would say it needs verifying, with no way out.
     * Reusing the row is safe because an unverified account holds no privileges
     * and only the real inbox owner can complete verification.
     *
     * @param user The user with profile details and a plaintext password in
     * {@code passwordHash}
     * @return The verification session handle, masked email, and code expiry
     * @throws IllegalArgumentException If a uid was supplied, required fields
     * are missing, or the email already belongs to a verified account
     */
    @Transactional
    public AccountVerificationStartResponse registerUser(RegisterUserRequest request) {
     ...
     *
     * <p>
     * On success the account is marked verified, the code is discarded, and a
     * JWT is returned so the frontend can log the user straight in rather than
     * making them retype credentials.
     *
     * @param req The verification session handle and the submitted code
     * @return A JWT for the now-verified user
     * @throws IllegalArgumentException If the request is malformed, the session
     * is unknown, the code is expired, too many wrong guesses have been made,
     ...
     *
     * <p>
     * Keyed by email rather than by verification id, because the user this
     * exists for is exactly the one who closed the tab and no longer has a
     * verification id to send back.
     *
     * @param req The email address of the pending account
     * @return A new verification session handle, masked email, and code expiry
     * @throws IllegalArgumentException If the email is unknown, already
     * verified, or a code was sent too recently
     */
    @Transactional
     ...
     *
     * <p>
     * Verifies that the email exists, email is verified, and password matches.
     ...
     * @throws IllegalArgumentException If email/password invalid or email not
     * verified
     */
     ...
     *
     * <p>
     * Generates a reset token with 30-minute expiry and sends it via email to
     * the user.
     *
     * @param email The user's email
     ...
     *
     * <p>
     * Validates that the token exists and has not expired before updating the
     * password. Clears the reset token after successful password reset.
     *
     ...
     *
     * <p>
     * Null fields are left untouched, so a caller sending only {@code phone}
     * doesn't blank out the user's name.
     *
     * @param email The authenticated user's email
     ...
     *
     * <p>
     * The previous image is deleted from storage after the new URL is saved, so
     * an orphaned file isn't left paying for itself in S3 forever.
     *
     * @param email The authenticated user's email
     ...
     */
    @Transactional
    public UserProfileResponse updateAvatar(String email, MultipartFile image) {
    ...
    /**
     * Removes the user's avatar, falling back to whatever placeholder the
     * frontend shows.
     *
     * @param email The authenticated user's email
     * @return The updated profile
     ...
     * @throws IllegalArgumentException If the current password is wrong or the
     * new one matches it
     */
    @Transactional
     ...
     *
     * <p>
     * The account's live email is left alone until the code comes back, so a
     * mistyped address can't strand the user. Requires the current password,
     * because whoever controls the email controls password resets and therefore
     * the account.
     *
     ...
     * @throws IllegalArgumentException If the password is wrong, the address is
     * unchanged, the address is already taken, or a code was requested too
     * recently
     */
    @Transactional
     ...
     *
     * <p>
     * Returns a fresh JWT: the existing token identifies the user by their old
     * email, so it stops resolving the moment the address changes. The frontend
     * must swap in the returned token or the user will appear logged out on
     * their next request.
     *
     * @param email The authenticated user's current email
     * @param req The code sent to the pending address
     * @return A JWT issued against the new email
     * @throws IllegalArgumentException If there is no pending change, it
     * expired, too many wrong guesses were made, the code doesn't match, or the
     * address was claimed in the meantime
     */
    @Transactional(dontRollbackOn = IncorrectVerificationCodeException.class)
    ...
    // ── Helpers ────────────────────────────────────────────────
    /**
     * Stamps a fresh verification session onto the account and returns the
     * plaintext code.
     *
     * <p>
     * The caller is responsible for saving the user and queuing the email. Only
     * the hash is kept on the entity — the plaintext is returned once, here,
     * and never stored.
     */
    ...
    /**
     * Best-effort cleanup of a replaced avatar. A storage hiccup here shouldn't
     * fail the user's request — they've already got their new picture; the old
     * file is just litter.
     */
     ...
     *
     * <p>
     * Marked on the transaction via {@code dontRollbackOn}; without it the
     * increment would be rolled back with the exception and the attempt cap
     * would never bite.
     */
