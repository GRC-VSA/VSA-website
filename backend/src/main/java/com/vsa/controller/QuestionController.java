...
public class QuestionController {
    ...
    @PostMapping
    public ResponseEntity<List<Question>> createQuestions(@PathVariable Long eventId, @Valid @RequestBody List<@Valid QuestionRequest> requests){
        return ResponseEntity.status(HttpStatus.CREATED).body(questionService.createQuestions(eventId, requests));
    }

    @PutMapping("/{questionId}")
    public ResponseEntity<Question> updateQuestion(@PathVariable Long eventId, @PathVariable Long questionId, @Valid @RequestBody QuestionRequest request){
